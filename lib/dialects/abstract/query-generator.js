  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    var table = null
      ...
      , subQuery = (options.doSubQuery === true) && (options.subQuery === undefined ?
    ...
    if (options.tableAs) {
      ...
      if (!options.as) {
        options.as = mainTableAs;
      }
    } else if (options.as) {
      options.tableAs = options.as;
      mainTableAs = this.quoteTable(options.tableAs);
      options.as = mainTableAs;
    ...
    if (subQuery || options.groupedLimit) {
      ...
      // fixed by yuriscom@gmail.com
      subQueryAttributes = [(mainTableAs || table) + '.*'];
      ...
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (options.includeIgnoreAttributes !== false) {
          ...
          if (include.subQuery && subQuery) {
            ...
            // fixed by yuriscom@gmail.com
            mainAttributes = mainAttributes.concat(include.attributes.map(function(attr){
              return (mainTableAs || table) + "." + self.quoteIdentifier(as + '.' + attr, true);
            }))
            //=========
    ...
    if (subQuery) {
      query = 'SELECT ' + (options.distinct===true ? 'DISTINCT ' : '') + mainAttributes.join(', ') + ' FROM (';
  ...
  selectFromTableFragment: function(options, model, attributes, tables, mainTableAs, whereClause) {
    var fragment = 'SELECT ' + (options.distinct===true ? 'DISTINCT ' : '') + attributes.join(', ') + ' FROM ' + tables;
